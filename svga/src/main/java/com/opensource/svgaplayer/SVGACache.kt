...
object SVGACache {
    ...
    fun buildCacheKey(str: String): String {
        return try {
            ...
            buildCacheKeyWithFormat(str)
        }
    }

    private fun buildCacheKeyWithFormat(str: String): String {
        val messageDigest = MessageDigest.getInstance("MD5")
        messageDigest.update(str.toByteArray(charset("UTF-8")))
        val digest = messageDigest.digest()
        var sb = ""
    ...
    fun buildCacheDir(cacheKey: String): File {
        LogUtils.info("SVGAParser", "path = ${File("$cacheDir$cacheKey/").absolutePath}")
        return File("$cacheDir$cacheKey/")
    }

    fun buildSvgaFile(cacheKey: String): File {
        LogUtils.info("SVGAParser", "path = ${File("$cacheDir$cacheKey.svga").absolutePath}")
